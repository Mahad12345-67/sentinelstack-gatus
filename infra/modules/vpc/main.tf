...
# NAT GW (regional — AWS auto-provisionerar EIP:er och AZ-täckning)
...
# FIX: NAT-gateway-rutten använder nat_gateway_id (inte gateway_id, som i Suads original)
resource "aws_route_table" "Private-RT" {
  vpc_id = aws_vpc.vpc-gatus.id

  route {
    cidr_block     = var.internet-cidr
    nat_gateway_id = aws_nat_gateway.nat-gw-gatus.id
  }
